...
_start:
    ...
    ; y=((22*3-2*x)*2)/(5+6)-3
    mov rbx, 2
    imul rbx                   ; rax(x) * rbx(2)
    neg rax                    ; -rax(x * 2)
    add rax, 66                ; -rax(x * 2) + 66
    imul rbx                   ; (-rax(x * 2) + 66) * rbx(2)
    mov rbx, 11
    idiv rbx                   ; ((-rax(x * 2) + 66) * rbx(2)) / 11
    add rax, -3                ; ((-rax(x * 2) + 66) * rbx(2)) / 11 - 3
...
str_to_num:
    ...
    mov dl, [rcx]        ; bl = символ который используется сейчас
